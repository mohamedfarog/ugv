...
class CarGazeboPlugin : public gazebo::ModelPlugin {
 ...
 private:
  ...
  double steer = 0;
  double velocity = 0;
  const double wheelbase_length = 0.7429;
  const double front_wheelbase_width = 0.90;
  const double rear_wheelbase_width = 0.90;
  const double shock_p = 500;
  const double shock_d = 60;
  const double wheel_diameter = .336;
  const double max_speed = 5;
  const double max_turn_angle = M_PI/8.0;
  AckermannModel car_model = {0.90, 0.90};
